...
class EventsRepository {
  ...
  Future<Scence> createNewScence({required String name,required String? description,required List<Event> events}) async {
    try {
      final uri = EventsNetworking.createScence;
      final customOptions = await getCustomOptions();
      final response = await _client.postUri(
        uri,
        data: {"name": name,"description":description,"events":(events.map((e) => e.toJson()).toList())},
        options: commonOptionsWithCustom(customOptions: customOptions),
      );
      final data = response.data['group'];
      final group = Scence.fromJson(data);
      return group;
    } on DioError catch (e) {
      final error = decodeDioError(e);
      throw error;
